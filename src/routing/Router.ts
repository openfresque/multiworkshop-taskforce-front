...
export type SlottedTemplateResultFactory = (
  subViewSlot: TemplateResult
) => TemplateResult

export type ViewChangedCallback = (
  templateResultCreator: SlottedTemplateResultFactory,
  path: string
) => void
...
type RouteDeclaration = {
  ...
  viewContent: (
    pathParams: Record<string, string>
  ) => Promise<SlottedTemplateResultFactory> | SlottedTemplateResultFactory
...
class Routing {
  ...
  private static readonly DEFAULT_TITLE =
    'Trouver Une Fresque : trouvez une fresque près de chez vous'
  ...
  installRoutes(
    callback?: ViewChangedCallback
  ): ViewChangedCallbackCleaner | undefined {
    ...
    this.declareRoutes({
      ...
      viewContent: async () => {
        ...
        return subViewSlot => html`<tuf-home>${subViewSlot}</tuf-home>`
      },
    })
    ...
    this.declareRoutes({
      ...
      pageTitleProvider: params =>
        State.current
          .chercheDepartementParCode(params[`codeDpt`])
          .then(
            nomDpt =>
              `Fresques en ${nomDpt.nom_departement} ${params[`codeDpt`]}`
          ),
    })
    ...
    this.declareRoutes({
      ...
      pageTitleProvider: params =>
        State.current
          .chercheCommuneParCode(params['codePostal'], params['codeCommune'])
    ...
    this.declareRoutes({
      ...
      viewContent: async () => {
        ...
        return subViewSlot => html`<tuf-map>${subViewSlot}</tuf-map>`
      },
    })
    this.declareRoutes({
      pathPattern: `/apropos`,
      viewContent: async () => {
        ...
        return subViewSlot => html`<tuf-apropos>${subViewSlot}</tuf-apropos>`
      },
    })
    this.declareRoutes({
      ...
      viewContent: async () => {
        ...
        return subViewSlot => html`<tuf-mentions>${subViewSlot}</tuf-mentions>`
      },
    })
  ...
  private declareRoutes(routeDeclaration: RouteDeclaration) {
    const paths: string[] =
      typeof routeDeclaration.pathPattern === 'string'
        ? [routeDeclaration.pathPattern]
        : routeDeclaration.pathPattern
    paths.forEach(path => {
      this._declareRoute(
        path,
  ...
  private _declareRoute(
    ...
    page(
      `${this.basePath}${path.substring(path[0] === '/' ? 1 : 0)}`,
      context => {
        const slottedViewComponentFactoryResult = viewComponentCreator(
          context.params
        )
        ...
        Promise.all([
          ...
          this._viewChangeCallbacks.forEach(callback =>
            callback(slottedViewTemplateFactory, path)
          )
        })
      }
    )
  ...
  onViewChanged(callback: ViewChangedCallback): ViewChangedCallbackCleaner {
    ...
    return () => {
      const idx = this._viewChangeCallbacks.findIndex(
  ...
  private static _notFoundRoute(context: PageJS.Context) {
    ...
    console.error(
      `Route not found : ${context.path} ! Redirecting to ${notFoundUrl}`
    )
    window.location.href = notFoundUrl
  }

  public navigateToRendezVousAvecDepartement(
    codeDepartement: string,
    pathLibelleDepartement: string,
    searchType: SearchType,
    online: boolean
  ) {
    page(
      this.getLinkToRendezVousAvecDepartement(
        codeDepartement,
        pathLibelleDepartement,
        searchType,
        online
      )
    )
  ...
  public getLinkToRendezVousAvecDepartement(
    codeDepartement: string,
    pathLibelleDepartement: string,
    searchType: SearchType,
    online: boolean
  ...
  public navigateToRendezVousAvecCommune(
    codeTriCentre: CodeTriCentre,
    codeDepartement: string,
    pathLibelleDepartement: string,
    codeCommune: string,
    codePostal: string,
    pathLibelleCommune: string,
    searchType: SearchType,
    online: boolean
  ) {
    page(
      `${this.basePath}dpt${codeDepartement}-${pathLibelleDepartement}/commune${codeCommune}-${codePostal}-${pathLibelleCommune}/recherche-${searchTypeConfigFor(searchType).pathParam}/en-triant-par-${codeTriCentre}/online-${online ? 'oui' : 'non'}`
    )
  }
